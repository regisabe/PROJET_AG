...
```
.
├── index.html            → application principale (après connexion), CSS intégré
├── login.html             → page de connexion / création de profil, CSS intégré
...
├── js/
│   ├── common.js            → fonctions partagées (stockage, sécurité, helpers) utilisées par les deux pages
│   ├── login.js              → logique de la page de connexion
│   └── app.js                → toute la logique de l'application principale (React)
├── icons/                    → icônes de l'app (écran d'accueil, favicon, etc.)
└── .nojekyll                 → désactive le traitement Jekyll sur GitHub Pages
```

Chaque page HTML (`index.html`, `login.html`) contient son propre style
`<style>` intégré — il n'y a pas de fichier CSS séparé. Le code JavaScript,
lui, est partagé au maximum via `js/common.js` pour éviter les doublons.

**Comment ça s'articule :**
- On arrive toujours sur `login.html` en premier (ou on y est redirigé
  automatiquement si on n'est pas connecté).
- Après connexion (ou création de profil), on est redirigé vers `index.html`,
  qui contient l'application complète (tableau de bord, membres, paiements…)
  avec la navigation habituelle par le menu latéral.
- La déconnexion ramène automatiquement vers `login.html`.

L'app est toujours "sans build" : React, ReactDOM et Babel sont chargés
depuis un CDN, et les fichiers `js/*.js` sont transformés dans le navigateur.
Aucune étape de compilation n'est nécessaire pour déployer.
